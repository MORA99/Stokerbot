...
extern bool DS18B20Conv;
extern uint8_t start_web_client;
static uint8_t nextSensorPos = 0;

//Every other call a reading and every other call gets the result
...
uint8_t DS18X20_start_meas( uint8_t with_power_extern, uint8_t id[])
{
	...
	uint8_t DS18X20_read_meas(uint8_t id[], uint8_t *subzero, uint8_t *cel, uint8_t *cel_frac_bits , uint16_t *maalt)
	{
		uint8_t i;
